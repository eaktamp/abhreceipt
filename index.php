                       ...
                       <div class="input-group col-lg-8 mb-4">
                        ...
                        <div class="input-group col-lg-12    mb-4">
                            ...
                            <select id="object_name" name="object_name" class="form-control custom-select bg-white border-left-0 border-md" required>
                                ...
                                <option value="3">เพื่อใช้ใน รพ สนาม (สามารถระบุเพิ่มเติมได้)</option>
                                <option value="4">เพื่อใช้พัฒนาโรงพยาบาล (สามารถระบุเพิ่มเติมได้)</option>
                            </select>
                        </div>
                        <!--  -->
                        <div class="input-group col-lg-12 mb-4">
                        ...
                        <div class="input-group te col-lg-5 col-5 col-md-12 mb-4 mt-3">
                            ...
                            &nbsp;&nbsp;&nbsp; -->
                            <input type="radio" name="transfer_bank" id="scb"  class="old" value="scb"  required checked/>
        ...
        if ($check !== false) { //เช็คใช่ไฟล์ภาพหรือไม่
            ...
            if ($uploadOk == 0) {
                ...
                if (move_uploaded_file($_FILES["file_upload"]["tmp_name"], $target_file)) {
                    htmlspecialchars(basename($_FILES["file_upload"]["name"])) . " has been uploaded.";
                     $pt_name = $_POST['company_name'];
                     $cid = $_POST['id_card'];
                     $address_number_moo = $_POST['addr'];
                     $tmp_name = $_POST['district'];
                     $amp_name = $_POST['amphoe'];
                     $chw_name = $_POST['province'];
                     $zipcode = $_POST['zipcode'];
                     $phone   =  $_POST['phone'];
                     $amount  =  $_POST['amount'];
                     $transfer_monny = str_replace(",", "", $amount);

                     $object_name = $_POST['object_name'];
                     $object_orther = $_POST['cc'];
                     $transfer_bank = $_POST['transfer_bank'];
                     $transfer_date = $_POST['startdate'];
                     $transfer_time = $_POST['enddate'];
                     $img_name =  $newimgname . '.' . $imageFileType;
                     $type_check = 'N';
                     $ipupdate  = get_client_ip();
                    ...
                    $only_country = explode(" ", $country);
                     ...
                     $localtion_check = $only_country[1] . " " . substr($only_country[2], 0, 4);



                   
                    $sql = "INSERT INTO abhrcdb
            (  pt_name,cid,phone,object_name,object_orther,transfer_bank,transfer_date,transfer_time,
             ...
             type_check,flage,ipupdate , device_send ,macupdate,localtion_check,gender)
             VALUES ('$pt_name','$cid','$phone','$object_name','$object_orther','$transfer_bank','$transfer_date','$transfer_time','$transfer_monny',
             '$img_name','$address_number_moo','$tmp_name','$amp_name','$chw_name','$zipcode','$type_check','Default','$ipupdate','$device_send','$macupdate','$localtion_check','$gender') ";
